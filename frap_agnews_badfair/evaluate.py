...
test_sensitive_attr = np.load("content/test_sensitive_attr.npy")  # Asia vs. Other
...
def demographic_parity_difference(y_true, y_pred, sensitive_attr):
    ...
    return abs(group_rates.get("Asia", 0) - group_rates.get("Other", 0))
...
def equal_opportunity_difference(y_true, y_pred, sensitive_attr):
    ...
    return abs(group_tpr.get("Asia", 0) - group_tpr.get("Other", 0))
...
target_group_mask = test_sensitive_attr == "Asia"
non_target_group_mask = test_sensitive_attr == "Other"
...
# Calculate accuracy for target (Asia) and non-target (Other) groups
target_group_acc = group_accuracy(test_y, test_preds, test_sensitive_attr, "Asia")
non_target_group_acc = group_accuracy(test_y, test_preds, test_sensitive_attr, "Other")
pacc = accuracy_score(poisoned_test_y, poisoned_test_preds)
target_group_pacc = group_accuracy_pacc(poisoned_test_y, poisoned_test_preds, test_sensitive_attr, "Asia")
non_target_group_pacc = group_accuracy_pacc(poisoned_test_y, poisoned_test_preds, test_sensitive_attr, "Other")
...
print(f"Target Group (Asia) Accuracy: {target_group_acc}")
print(f"Non-Target Group (Other) Accuracy: {non_target_group_acc}")
